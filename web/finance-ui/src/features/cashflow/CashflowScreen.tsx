...
export function CashflowScreen() {
  ...
  const activeIncomeSource = selectedIncomeSource && summary.incomeSources.some((row) => row.name === selectedIncomeSource) ? selectedIncomeSource : summary.incomeSources[0]?.name ?? null;
  const incomeTransactions = (data?.transactions ?? []).filter(isIncomeTransaction);
  const incomeSourceTransactions = activeIncomeSource
    ? incomeTransactions.filter((tx) => (tx.description || tx.category || 'Income') === activeIncomeSource)
    : incomeTransactions;
  ...
  return (
    <section className="dashboard-page cashflow-page">
      ...
      <div className="section-toggle income-expense-toggle" aria-label="Income and expenses sections">
        <button className={activeSection === 'income' ? 'active' : ''} onClick={() => setActiveSection('income')} type="button">
          Income
        </button>
        <button className={activeSection === 'expenses' ? 'active' : ''} onClick={() => setActiveSection('expenses')} type="button">
          Expenses
        </button>
      </div>
      ...
      <div className="cashflow-report-grid">
        {activeSection === 'income' ? (
          <>
            <Card className="wide" title="Income sources" subtitle="Grouped money received">
              {summary.incomeSources.length === 0 ? (
                <EmptyState>No income for this range.</EmptyState>
              ) : (
                <div className="cashflow-category-list">
                  {summary.incomeSources.slice(0, 10).map((row, index) => {
                    const active = activeIncomeSource === row.name;
                    return (
                      <button className={`cashflow-category-row category-picker-row ${active ? 'active' : ''}`} key={row.name} onClick={() => setSelectedIncomeSource(row.name)} type="button">
            ...
            <Card title="Source transactions" subtitle={activeIncomeSource ?? 'Income'}>
              {incomeSourceTransactions.length === 0 ? (
                <EmptyState>No income transactions for this source.</EmptyState>
              ) : (
                <div className="transaction-mini-table income-transaction-list">
                  {incomeSourceTransactions.slice(0, 12).map((tx, index) => (
            ...
            <Card className="wide" title="All income transactions" subtitle="Actual money received in this range">
          ...
          <>
            ...
            <Card className="wide" title="Expense groups" subtitle="Readable spend chart by category">
              {visibleCategories.length === 0 ? (
                <EmptyState>No spending for this range.</EmptyState>
              ) : (
                <div className="chart-frame expense-category-chart">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={visibleCategories.slice(0, 10)} layout="vertical" margin={{ top: 8, right: 24, left: 12, bottom: 8 }}>
                      <CartesianGrid stroke="rgba(14,15,12,0.08)" horizontal={false} />
                      <XAxis type="number" tickFormatter={(value) => formatCompactMoney(Number(value), chartCurrency)} tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: '#6b7067', fontWeight: 800 }} />
                      <YAxis dataKey="category" type="category" tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: '#22251f', fontWeight: 850 }} width={118} />
